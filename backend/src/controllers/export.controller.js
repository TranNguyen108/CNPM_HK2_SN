...
const { renderTaskReportPdf, renderCommitReportExcel } = require('../services/exportReport.service');

const DEFAULT_TASK_FORMAT = 'pdf';
const DEFAULT_COMMIT_FORMAT = 'excel';
...
exports.exportTaskReport = async (req, res) => {
  try {
    ...
    const format = String(req.query.format || DEFAULT_TASK_FORMAT).trim().toLowerCase();
    if (format !== 'pdf') {
...
exports.exportCommitReport = async (req, res) => {
  try {
    ...
    const format = String(req.query.format || DEFAULT_COMMIT_FORMAT).trim().toLowerCase();
    ...
    if (req.query.from && !from) return res.status(400).json({ message: 'from must be a valid date' });
    if (req.query.to && !to) return res.status(400).json({ message: 'to must be a valid date' });
    if (from && to && from > to) return res.status(400).json({ message: 'from must be earlier than or equal to to' });
